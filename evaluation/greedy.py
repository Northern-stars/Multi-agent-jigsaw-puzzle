...
HORI_SCORE_NAME=f"dataset/hori_score_{MODEL_NAME}.npy"
VERT_SCORE_NAME=f"dataset/vert_score_{MODEL_NAME}.npy"
CATE_SCORE_NAME="dataset/cate_score_9.npy"
HORI_MODEL_PATH=f"model/hori_{MODEL_NAME}.pth"
VERT_MODEL_PATH=f"model/vert_{MODEL_NAME}.pth"

# model=pretrain_model(256,256).to(DEVICE)
hori_model=pretrain_model(512,512,fen=MODEL_NAME).to(DEVICE)
vert_model=pretrain_model(512,512,fen=MODEL_NAME).to(DEVICE)
...
def load_cate_matrix():
    if os.path.exists(CATE_SCORE_NAME):
        ...
        cate_model.eval()
        cate_matrix=torch.zeros([1000,18,9])
        for index in range(test_x.shape[0]//2):
            if index>=666:
                i=index+667
            elif index>=333:
...
if __name__=="__main__":
    ...
    greedy_test(hori_score,vert_score,cate_score,True)
