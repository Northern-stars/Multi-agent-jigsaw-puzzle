...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        for c_rgb, c_noise, x_rgb, x_noise, label in pbar:
            c_rgb, c_noise = c_rgb.to(DEVICE), c_noise.to(DEVICE)
            x_rgb, x_noise = x_rgb.to(DEVICE), x_noise.to(DEVICE)
            label = label.to(DEVICE)

            optimizer.zero_grad()
            logits = model(c_rgb, c_noise, x_rgb, x_noise)
        ...
        with torch.no_grad():
            for c_rgb, c_noise, x_rgb, x_noise, label in tqdm(
                test_loader, desc="[Eval]"
            ):
                c_rgb_d, c_noise_d = c_rgb.to(DEVICE), c_noise.to(DEVICE)
                x_rgb_d, x_noise_d = x_rgb.to(DEVICE), x_noise.to(DEVICE)
                label_d = label.to(DEVICE)

                logits = model(c_rgb_d, c_noise_d, x_rgb_d, x_noise_d)
                preds = (logits > 0).float()
