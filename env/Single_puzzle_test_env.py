...
ACTOR_PATH=os.path.join("model/Actor_"+MODEL_NAME+".pth")
CRITIC_PATH=os.path.join("model/Critic_"+MODEL_NAME+".pth")
...
class Actor_model(nn.Module):
    def __init__(self,fen_hidden1,fen_hidden2,hidden1,hidden2,action_num,dropout=0.1):
        ...
        self.fen_model=Fen_model(fen_hidden1,fen_hidden2,model_name="modulator")
...
class env:
    ...
    def get_reward(self,permutation_list):
        ...
        return local_reward_list,done_list
    ...
    def update(self, show=False):
        ...
        # 1. 分批计算values和next_values
        memory_size = len(self.mkv_memory)
        all_states = []
        all_next_states = []
        all_image_indices = []
        
        # 先收集所有状态（这部分仍然需要，但values计算可以分批）
        for i in range(memory_size):
            state = self.get_image(self.mkv_memory[i]["State"], 
                                self.mkv_memory[i]["Image_index"])
            next_state = self.get_image(self.mkv_memory[i]["Next_state"], 
                                    self.mkv_memory[i]["Image_index"])
            all_states.append(state)
            all_next_states.append(next_state)
            all_image_indices.append(self.mkv_memory[i]["Image_index"])
        
        # 2. 分批计算values
        values = np.zeros(memory_size)
        next_values = np.zeros(memory_size)
        
        self.critic_model.eval()
        with torch.no_grad():
            # 分批处理states
            for start in range(0, memory_size, self.batch_size):
                end = min(start + self.batch_size, memory_size)
                batch_states = torch.cat(all_states[start:end], dim=0)
                batch_values = self.critic_model(batch_states).squeeze(-1).cpu().numpy()
                values[start:end] = batch_values
            
            # 分批处理next_states
            for start in range(0, memory_size, self.batch_size):
                end = min(start + self.batch_size, memory_size)
                batch_next_states = torch.cat(all_next_states[start:end], dim=0)
                batch_next_values = self.critic_model(batch_next_states).squeeze(-1).cpu().numpy()
                next_values[start:end] = batch_next_values
        
        # 3. 收集奖励和done标志
        rewards = np.array([mem["Reward"] for mem in self.mkv_memory])
        dones = np.array([mem["Done"] for mem in self.mkv_memory], dtype=np.float32)
        image_indices = np.array(all_image_indices)
        
        # 4. 分批计算GAE
        advantages = np.zeros(memory_size)
        returns = np.zeros(memory_size)
        
        # 为每个image并行计算GAE
        for image_idx in range(self.image_num):
            # 找到该image的所有经验索引
            ...
            traj_dones = dones[indices]
            
            # 计算该轨迹的next_values（考虑done状态）
            traj_next_values = np.zeros_like(traj_values)
            traj_next_values[:-1] = traj_values[1:]
            # 最后一个状态的next_value
            last_idx = indices[-1]
            traj_next_values[-1] = next_values[last_idx] if traj_dones[-1] == 0 else 0
            
            # 批量计算GAE
            deltas = traj_rewards + self.gamma * traj_next_values * (1 - traj_dones) - traj_values
            
            # 反向累积GAE
            gae = 0
            traj_advantages = np.zeros_like(traj_values)
            for t in reversed(range(len(traj_values))):
                gae = deltas[t] + self.gamma * self.lmb * (1 - traj_dones[t]) * gae
                traj_advantages[t] = gae
            
            # 存储结果
            advantages[indices] = traj_advantages
            returns[indices] = traj_advantages + traj_values
        ...
        for epoch in range(self.epochs):
            # 每个epoch重新打乱
            shuffle_indices = np.arange(memory_size)
            ...
            for start in range(0, memory_size, self.batch_size):
                ...
                for idx in batch_indices:
                    mem = self.mkv_memory[idx]
                    state = self.get_image(mem["State"], mem["Image_index"])
                    batch_states.append(state)
                    batch_actions.append(mem["Action"])
                    batch_old_log_probs.append(mem["Log_prob"])
                    batch_advantages.append(advantages[idx])
                    batch_returns.append(returns[idx])
                
                # 转换为tensor
                state_tensor = torch.cat(batch_states, dim=0)
                action_tensor = torch.tensor(batch_actions).to(self.device).unsqueeze(-1)
                advantages_tensor = torch.tensor(batch_advantages, dtype=torch.float32).to(self.device).unsqueeze(-1)
                returns_tensor = torch.tensor(batch_returns, dtype=torch.float32).to(self.device).unsqueeze(-1)
                ...
                new_log_probs = dist.log_prob(action_tensor.squeeze()).unsqueeze(-1)
                entropy = dist.entropy().mean()
                
                # PPO核心损失
                ratio = torch.exp(new_log_probs - old_log_probs_tensor)
                surr1 = ratio * advantages_tensor
                surr2 = torch.clamp(ratio, 1-eps, 1+eps) * advantages_tensor
        ...
        # 显示训练信息
        if show and num_batches > 0:
            avg_actor_loss = total_actor_loss / num_batches
            avg_critic_loss = total_critic_loss / num_batches
            print(f"Update completed. Entropy weight: {self.entropy_weight:.4f}")
            print(f"Avg Actor Loss: {avg_actor_loss:.4f}, Avg Critic Loss: {avg_critic_loss:.4f}")
            print(f"Actor LR: {self.actor_optimizer.state_dict()['param_groups'][0]['lr']:.6f}")
            print(f"Critic LR: {self.critic_optimizer.state_dict()['param_groups'][0]['lr']:.6f}")
...
if __name__ == "__main__":
    ...
    critic_model=Critic_model(fen_hidden1=512,fen_hidden2=512,hidden1=1024,hidden2=512).to(DEVICE)
    actor_model=Actor_model(fen_hidden1=512,fen_hidden2=512,hidden1=1024,hidden2=512,action_num=28).to(DEVICE)
    model_fen_load(actor_model,"modulator")
    model_fen_load(critic_model,"modulator")
