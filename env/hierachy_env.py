...
class HierachyEnv:
    ...
    def load_image(self, image_num, id=None):
        if id:
            image_index = id
        else:
            ...
            for m in range(8):
                for n in range(8):
                    if permutation_raw[m][n] == True:
                        permutation_raw[m] = n + 1 if n >= 4 else n
                        break
    ...
    def get_image(self, permutation, image_index):
        ...
        for i in range(9):
            image[:, (0 + i // 3) * 96 : (1 + i // 3) * 96, (0 + i % 3) * 96 : (1 + i % 3) * 96] = self.permutation2piece[
                final_permutation[i]
            ]
    ...
    def get_reward(self, permutation_list):
        ...
        done_list = [0 for _ in range(len(permutation_copy))]
        local_reward_list = [0 for _ in range(len(permutation_copy))]
        consistency_reward_list = [0 for _ in range(len(permutation_copy))]
        ...
        for i in range(len(permutation_list)):
            ...
            piece_range = [0 for _ in range(len(permutation_list))]
            for j in range(piece_num):
                if permutation_copy[i][j] != -1:
                    piece_range[permutation_copy[i][j] // piece_num] += 1
                    local_reward_list[i] += (
                        permutation_copy[i][j] % piece_num == j and permutation_copy[i][j] // piece_num == i
                    ) * CATE_REWARD
    ...
    def summon_permutation_list(self, swap_num, id=None):
        if id:
            image_index = id
        else:
    ...
    def get_mask(self, permutation):
        ...
        for i in range(len(permutation)):
            if permutation[i] == -1:
                index = i + 1 if i >= self.piece_num // 2 else i
                mask.append(index)
        return mask
